...
export default function DriverPage() {
    ...
    // -- Map Ref: Required to move the map after search --
    ...
    // -- State for Map Bounds (Correctly scoped here!) --
    ...
    // --- FETCH DATA ---
    // Removed 'address' dependency to prevent refresh on every keystroke
    useEffect(() => {
        const fetchSpots = async () => {
            ...
            try {
                ...
                // This variable is now correctly accessible within the scope
                if (searchBounds) {
                    data = data.filter(s => {
                        // Convert to numbers to prevent text comparison errors
    ...
    const handlePlaceSelect = (place) => {
        if (place.geometry && place.geometry.location) {
            ...
            if (isBroadArea && place.geometry.viewport) {
                // City/Region - Use Google's Viewport
                ...
                // Use ref to actually move the map
                if (mapRef.current) {
                    mapRef.current.fitBounds(bounds);
                }

            } else {
                // Specific address - fixed radius
                const delta = 0.015; 
                setSearchBounds({
                    north: lat + delta,
                    south: lat - delta,
    ...
    // --- Combined Enter Logic (Legacy + New) ---
    const handleInputKeyDown = (e) => {
        // Legacy logic integration: Check if user is navigating with arrows
        ...
        if (e.key === 'Enter' && address.trim() !== '') {
            ...
            // Short delay to see if onPlaceChanged fires first
            setTimeout(async () => {
                if (isPlaceSelectedRef.current) return;
                
                // If nothing selected - manual fetch of the first result
                try {
        ...
        <div style={{ position: 'relative', height: '100dvh', width: '100vw', overflow: 'hidden', background: '#0b1220' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
                <MapComponent
                    ...
                    spots={realSpots}
                    center={mapCenter}
                    // Passing ref so we can control zoom
